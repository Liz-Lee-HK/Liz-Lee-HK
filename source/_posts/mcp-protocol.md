...
角色 | 运行位置 | 通信对象 | 核心职责 | 是否需要独立部署
------|----------|----------|----------|------------------
MCP 客户端 | 用户设备 / 服务器上（如 Claude App） | 主动连接所有 MCP 服务端 | 理解用户需求、发起工具调用、整理结果 | 是（独立应用）
MCP 服务端 | 后台 / 独立进程（如本地服务、云端服务） | 被动响应所有 MCP 客户端 | 提供数据、执行工具操作 | 是（独立服务）
MCP 应用 | 客户端内部的沙箱环境 | 只和客户端通信 | 渲染可视化界面、处理用户交互 | 否（依附于客户端）
